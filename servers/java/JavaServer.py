        ...
        try:
            ...
            if not INVALID_JAVA_SERVERS.is_invalid(self.ip): # do not log failed servers
                if type(e) == TimeoutError:
                    return logging.warning(f"Error grabbing server ERRORSPLIT{self.ip}: {ERRORS.get('Timeout')}")
                e = str(e)
                ...
                logging.warning(f"Error grabbing server ERRORSPLIT{self.ip}: {formated_error}")
            return
